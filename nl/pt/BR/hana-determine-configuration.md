...
As tabelas a seguir listam as configurações de {{site.data.keyword.baremetal_long}} disponíveis com a oferta {{site.data.keyword.cloud}} SAP-Certified Infrastructure. Para considerações adicionais ao executar o SAP HANA em um ambiente virtualizado, veja [Implementações do servidor VMware ESXi](/docs/infrastructure/sap-hana/hana-considerations.html#vmware-server).
...
Você é responsável por definir as configurações relacionadas ao VMware para seu servidor. É possível escolher entre três tamanhos, 1 TB (B1.S2.H4100 (VMware)), 2 TB (B1.S2.H4200 (VMware)) e 4 TB (B1.S2.4400 (VMware)). Para obter mais informações sobre como configurar o VMware em seu {{site.data.keyword.baremetal_short}} certificado pela SAP, veja [Diretrizes e melhores práticas de arquitetura para implementações do SAP HANA no VMware vSphere: guia de arquitetura e considerações técnicas](https://www.vmware.com/content/dam/digitalmarketing/vmware/en/pdf/whitepaper/sap_hana_on_vmware_vsphere_best_practices_guide-white-paper.pdf) (PDF).
